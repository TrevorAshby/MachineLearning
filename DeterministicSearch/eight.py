...
class Puzzle():
    ...
    def is_solved(self):
        ...
        solutionState = [0,1,2,3,4,5,6,7,8]
        if (self.state == solutionState):
            return True
   ...
   #     if (puzzle.state[i] == 0):
    #        #skip
     #       print("0")
      #  else:
       #     if (puzzle.state[i] == i):
           # Double loop, loop over the item we want to find
           #then loop over the current array of items till you find it
    for item in range(1, len(puzzle.state)):
        for arrayPos in range(0, len(puzzle.state)):
            #This if statement confirms that we are looking at the same
            #value in both for loops
            if (item == puzzle.state[arrayPos]):
                #if it is in the correct position, add nothing and continue
                if (item == arrayPos):
                    totalRow += 0
                    totalCol += 0  
                else:
                    #Else, check if the items moded by 3 are the same
                    #(If they are, then they are on the same column. Add
                    #no information)
                    if ((item % 3) == (arrayPos % 3)):
                        #do nothiing
                        totalCol += 0 
                    else:
                        totalCol += 1
                    #Check if the indicies divided by three are similar.
                    #If they are (both < 2 but > 1) then they are on the 
                    #same row, so add 0. Else, add one to the row value
                    itemFrac = item / 3
                    arrayPosFrac = arrayPos / 3
                    if (itemFrac and arrayPosFrac >= 2):
                        totalRow += 0
                    elif (itemFrac and arrayPosFrac >= 1):
                        totalRow += 0
                    elif (itemFrac and arrayPosFrac < 1):
                        totalRow += 0
                    else:
                        totalRow += 1
    return totalCol + totalRow #Add the number of value that it is off.
...
def manhattan_distance_to_goal(puzzle):
    ...
    manhattenVal = 0
    # YOUR TASK 1.4.2 CODE HERE
    #Manhatten value is the abs distance between two nodes. So
    # the Abs(arrayPos - item)
    for item in range(1, len(puzzle.state)):
        for arrayPos in range(0, len(puzzle.state)):
            if (item == puzzle.state[arrayPos]):
                #if it is in the correct position, add nothing and continue
                if (item == arrayPos):
                    ...
                    manhattenVal += abs(arrayPos - item)
    return manhattenVal
    #return 0 #change this!
...
def run_iterative_search(start_node):
    ...
    """
    if start_node.options.type == 'a':
        depth_limit = heuristic(start_node, options)
    ...
    #while depth_limit < max_depth_limit:
        ...
        path_length, ida_star_complete = run_dfs(start_node, depth_limit, visited) 
    
        #See how many nodes we expanded on this iteration and add it to our total
        total_expanded += visited['N']
        #print("Path length: " + str(path_length) + " ida* complete: " + str(ida_star_complete))
